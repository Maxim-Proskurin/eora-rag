"""
Индексация чанков в ChromaDB для GigaChat.
"""
...
from app.llm.gigachat.chunk import Chunk, split_text_by_tokens
from app.llm.gigachat.embeddings import GigaChatEmbeddingProvider
from app.scripts.chroma import get_chroma_client, get_or_create_collection


def index_chunks_in_chroma_gigachat(
    chunks: List[Chunk],
    collection_name: str = "eora_gigachat",
):
    """
    Индексирует чанки в ChromaDB для GigaChat.
    """
...
def _index_embeddings(collection, texts, metadatas, ids, logger):
    embeddings = GigaChatEmbeddingProvider().embed(list(texts))
    logger.info(f"Эмбеддинги для GigaChat рассчитаны за {time.time() - time.time():.1f} сек.")
    if not embeddings:
